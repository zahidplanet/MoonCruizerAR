...
namespace Niantic.ARDK.AR.WayspotAnchors.Editor
{
  ...
  internal class _VPSLocationManifestInspector: UnityEditor.Editor
  {
    ...
    private void DrawAnchorGUI(AuthoredWayspotAnchorData data)
    {
      ...
      GUILayout.BeginHorizontal();
      {
        EditorGUILayout.LabelField("Associated Prefabs", GUILayout.Width(_colOneWidth));
        EditorGUILayout.LabelField(data.AssociatedPrefabs == null ? "0" : data.AssociatedPrefabs.Count.ToString());
      }
      GUILayout.EndHorizontal();

      var hasBackingAnchor = !string.IsNullOrEmpty(data.Payload);
      GUILayout.BeginHorizontal();
      {
        EditorGUILayout.LabelField("Relative Position", GUILayout.Width(_colOneWidth));
        EditorGUILayout.LabelField(hasBackingAnchor ? data.Position.ToString() : CommonStyles.NaNVector);
      }
      GUILayout.EndHorizontal();
      ...
      GUILayout.BeginHorizontal();
      {
        ...
        EditorGUILayout.LabelField(hasBackingAnchor ? data.Rotation.ToString() : CommonStyles.NaNVector);
      }
      GUILayout.EndHorizontal();
